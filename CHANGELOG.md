...
- `analyze` accepts `--config` / `-c`, like `validate`. The GitHub Action already passed `--config` to `analyze` whenever `config-file` was set, which failed argument parsing; `settings.hide_severities` was read from nowhere and `--run-all-checks` ran on built-in defaults. The config is loaded once, before any Access Analyzer call, so a malformed file fails without spending API calls, and the same config drives the Access Analyzer filter, `--run-all-checks` and its PR comment
- `validate_policies(config=...)` accepts an already-loaded `ValidatorConfig`; `config_path` is ignored when it is given
...
- `policy_size.organizations_measurement: compact` measures SCPs and RCPs minified, for pipelines that minify before submitting (Terraform's `jsonencode`). The default, `as_written`, no longer counts a UTF-8 byte-order mark, which is not part of the submitted document
