...
- `get_condition_requirements_for_action` tool wrapper — its `get_condition_requirements()` helper is retained in `query.py` and will be re-exposed by a `describe_checks` tool in a follow-up, not by `query`.
