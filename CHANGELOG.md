...
- A `Dockerfile` for the hosted MCP server, running as a non-root user with AWS service data baked in at build time so the container makes no outbound calls. The operator must explicitly choose an auth mode and supply a config.
- `ServerSettings.http_max_connections`/`http_max_keepalive_connections` now configure the shared `AWSServiceFetcher`'s connection pool, instead of it silently using httpx's library defaults.
- `iam_validator.core.policy_checks.build_registry()`, extracted from `validate_policies()`, builds a check registry (built-in checks, config, custom-check loading/discovery) once for reuse. Exported from the SDK as `iam_validator.sdk.build_registry`.
- `validate_policies(..., registry=)`: when supplied, skips registry construction and config/custom-check loading, validating with the given registry instead. `registry=None` (default) is unchanged; the CLI and GitHub Action are unaffected.
- `iam_validator.mcp.settings.ServerSettings`: a Pydantic model resolving MCP server configuration (mode, transport, auth, resource limits, etc.) from `IAM_VALIDATOR_MCP_*` environment variables via `ServerSettings.from_env()`. Not yet wired into the server.
- `iam_validator.mcp.context.ServerContext`: built once at startup and held in the FastMCP lifespan, replacing the MCP server's module-level globals — no MCP tool reads one anymore. `ServerContext.mutable` (`SessionState`) holds session-scoped org config and custom instructions in local mode, and is `None` in hosted mode. The startup registry is reused across calls unless a per-call `config_path` or session override is active; custom checks default off in local mode and on in hosted mode for the operator's own config. The shared `AWSServiceFetcher` is prewarmed before the server reports ready.
- The `iam-validator-mcp` CLI and `iam-validator mcp` command bridge their flags (`--config`, `--instructions`, `--instructions-file`, `--custom-checks-dir`, `--aws-services-dir`) to `IAM_VALIDATOR_MCP_*` environment variables, resolved once at lifespan startup by `ServerSettings.from_env()`.
- `iam_validator.mcp.component_spec.ComponentSpec` (`ToolSpec`/`ResourceSpec`/`PromptSpec`): declarative gating metadata (`tag`, `modes`, `transports`, `scopes`, `mutating`) shared by tools, resources, and prompts, so a profile or transport restriction can't be bypassed via an equivalent resource. `build_server(settings)` filters every spec through `spec_survives()` and registers survivors on a fresh `FastMCP` instance. `read-only` now filters on `mutating` instead of `tag`.
...
- Hosted-mode MCP config is now resolved once at startup from `--config`/`IAM_VALIDATOR_MCP_CONFIG` into an immutable `ServerContext.config`; a missing, unreadable, or invalid config, or a custom check that fails to import, now fails startup loudly (`HostedStartupError`, non-zero exit) instead of falling back to defaults. Local mode's warn-and-continue behavior is unchanged. `custom_instructions` is also read at startup with no reload short of a redeploy. The 5 session-mutating `orgconfig` tools are structurally excluded from hosted mode; `get_organization_config` now reports the hosted baseline instead of `has_config: False`.
- `ServerContext.config_digest`: a stable SHA-256 over the resolved config plus the built registry's `(check_id, source, enabled, severity)` tuples, so a check added via an installed entry-point distribution still changes the digest. `CheckRegistry.register()` now takes a `source` and exposes `get_source(check_id)`. Returned in `get_config`/`get_organization_config` and every `validate_policy`/`validate_policies_batch` response.
- `iam_validator.core.policy_checks.overlay_registry_config(base_registry, config)`: builds a new registry reusing `base_registry`'s check instances under a different `ValidatorConfig`, so an inline or session config override no longer re-imports custom checks/entry points on every call.
- `iam_validator.mcp.tools.validate.validate_policies`: a single `validate` tool absorbing six legacy tools (`validate_policy`, `quick_validate`, `validate_policies_batch`, `validate_with_config`, `check_org_compliance`, `get_policy_summary`). Takes `policies`, `policy_type`, `detail` (`summary`/`findings`/`full`), and `format` (`console`/`enhanced` rejected). `path`/`glob` are local-mode only; `name` is an opaque label, never opened as a path. Results carry `is_valid`, `fails_policy`, `severity_counts`, `policy_type`/`policy_type_source`, `name`; a `report` field is added when `format != "json"`, `results` is otherwise byte-identical, and every response carries `config_digest`.
- Five `ServerSettings` request limits (`max_policies=50`, `max_policy_bytes=1 MiB`, `max_request_bytes=8 MiB`, `request_timeout_s=60`, `max_response_bytes=4 MiB`), enforced before validation work. The first four raise `ToolError` naming the limit and observed value; `max_response_bytes` instead degrades `detail` toward `summary` and sets `truncated`/`truncated_count`.
- `iam_validator.mcp.tools.query.query`: a single `query` tool absorbing seven legacy tools (`query_service_actions`, `query_action_details`, `query_actions_batch`, `check_actions_batch`, `query_condition_keys`, `query_arn_formats`, `expand_wildcard_action`). Takes a `kind` (`service_actions`/`action_details`/`condition_keys`/`arn_formats`/`expand_wildcard`) plus its parameters; `actions`/`patterns` are lists, so batch variants are just multi-entry calls, returned in request order. `inputSchema` requires the right parameter per `kind`; invalid input raises `ToolError`, so hosted `mask_error_details` stays actionable. `output_schema` is a `oneOf` built from the response models.
- `iam_validator.mcp.tools.checks.describe_checks` (tag `validate`): a single check-catalog tool absorbing `get_issue_guidance`. Takes an optional `check_ids` filter and reports, per check: `check_id`, `description`, `default_severity`, class docstring, `applies_to_policy_types`, `supersedes`, resolved non-`enabled`/`severity` config options, and provenance `source`. Refuses to import under `-OO`/`PYTHONOPTIMIZE=2`, which strips the docstrings this tool depends on.
- `iam_validator.mcp.tools.config.get_config` (tag `orgconfig`, always available): absorbs `get_organization_config`, `get_active_profile` and `get_custom_instructions` into one read, returning the effective config, its source, `config_digest`, active `mode`/`profile`, live `tool_count`/`tool_names`, and `custom_instructions`.
- `iam_validator.mcp.tools.config.set_config` (tag `orgconfig`, `mutating`, local/stdio only): absorbs `set_organization_config`, `clear_organization_config`, `load_organization_config_from_yaml`, `set_custom_instructions` and `clear_custom_instructions` into one call taking `config`/`yaml_content`/`clear_config` and `instructions`/`clear_instructions`. Unlike other consolidated tools, it edits only in-process `SessionState`, with no CLI/SDK counterpart.

- `iam_validator.mcp.tools.analyze.analyze_policy` (tag `analyze`): renamed from `aws_access_analyzer_validate`, with mode-gated variants — local carries `profile`, hosted omits it from its schema. `region` is now checked against `ServerSettings.allowed_regions` (default: the server's own region) before a boto3 session is created; an out-of-allowlist region raises `ToolError`. A new `AnalyzeRateLimiter`, sized from `ServerSettings.analyze_rate_limit` (default 10/min, `0` disables), caps calls per worker process with a best-effort in-process sliding-window counter — not a quota. It remains the only tool with `openWorldHint=True`.
- An AWS Lambda entry point (`iam_validator.mcp.awslambda`, `[lambda]` extra, `docker build --target lambda`) for running the hosted MCP server behind a Function URL or API Gateway, plus a `--auth aws-gateway` provider that trusts the adapter's already-verified request context instead of a bearer token.
- `docs/integrations/mcp-lambda.md`: deployment docs for the Lambda entry point, covering auth shapes, `/tmp` and JWKS caching, timeouts, cold starts, and a worked Function URL deployment.
- `iam_validator.mcp.audit`: hosted mode emits one structured JSON log record per `validate_policies`/`analyze_policy`/`query`/`describe_checks`/`get_config` call, via `audited_call()` (a no-op outside hosted mode). Each record carries `timestamp`, `tool`, `subject`, `scopes`, `config_digest`, `policy_count`, `duration_s`, `outcome`, `severity_counts` — never the submitted policy or a finding message. `analyze_policy` attributes it to the caller's subject even though the call spends the server's own AWS credentials. A failure emitting it is logged separately and never fails the call; a cancelled call gets a distinct `cancelled` outcome.
- `iam_validator/mcp/server.py` (the 1608-line monolithic FastMCP server) is dissolved: its 24 tools, 7 resources, and 3 prompts are ported unchanged in behaviour into `iam_validator.mcp.tools.{validate,query,config,analyze}`, `resources.RESOURCES`, and `prompts.PROMPTS`. `build_server()` is now the sole server-construction path for `create_server()`/`run_server()` and the `--profile`/`--list-profiles` CLI flags. `get_active_profile` now reads the profile from `ServerContext.settings` instead of a module-level global.
- `iam_validator.mcp.auth.get_auth_provider(settings, *, token_cli_flag=None)`: builds the FastMCP `AuthProvider` selected by `ServerSettings.auth` — none, static token, JWT, or one of six IdP providers (`azure`/`google`/`github`/`keycloak`/`auth0`/`workos`). All provider material is read from `IAM_VALIDATOR_MCP_AUTH_*` environment variables or a file path they name, never a CLI flag. It also independently rejects hosted mode running without explicit `--auth none`. Every failure prints to stderr and raises `SystemExit(1)`. Also adds `SCOPE_TO_TAG` (scope -> tag mapping for gating) and `SCOPE_FREE_TAGS` (tags left ungated).
- `build_server()` now wires `get_auth_provider(settings)` into the `FastMCP` instance's `auth=` and attaches a per-component scope check to every tool, resource, and prompt whose tag maps to a scope. A caller lacking the required scope gets the component hidden from listings rather than an explicit authorization error, which would disclose the scope taxonomy. Only takes effect when `settings.auth != "none"`.
- `iam_validator.mcp.cli`: the shared argparse layer both `iam-validator-mcp` and `iam-validator mcp` now build their parser from, so both entry points resolve identical flags to an identical `ServerSettings`. Every flag mirrors an `IAM_VALIDATOR_MCP_*` environment variable, with flags winning over env vars and env vars winning over defaults. `--auth` only selects a provider by name — no flag accepts token/JWT/IdP material. A validation-ordering fix stops a valid flag+env combination (e.g. `IAM_VALIDATOR_MCP_MODE=hosted` with `--auth none`) from being wrongly rejected by the hosted-mode-requires-explicit-auth check.
...
- The `_temp_config_file` temp-YAML-dump hot path in `mcp/tools/validate.py`, and the equivalent inline `tempfile.NamedTemporaryFile` in `mcp/tools/config.py` — both now pass a `ValidatorConfig`/dict straight through and reuse the startup registry via `overlay_registry_config()` instead of rebuilding it.
...
- `query_service_actions`, `query_action_details`, `query_actions_batch`, `check_actions_batch`, `query_condition_keys`, `query_arn_formats` and `expand_wildcard_action` tools — superseded by the single `query` tool above. The MCP server now exposes 12 tools (previously 19).
- `get_condition_requirements_for_action` tool wrapper — its underlying helper is retained in `query.py`; the per-action lookup it exposed is superseded by `describe_checks`'s per-check resolved `config` field.
- `get_issue_guidance`, `check_sensitive_actions` tools — superseded by `describe_checks` (`check_sensitive_actions`'s functionality had already moved to `query`'s `action_details` kind). `set_organization_config`, `get_organization_config`, `clear_organization_config`, `load_organization_config_from_yaml`, `set_custom_instructions`, `get_custom_instructions`, `clear_custom_instructions` and `get_active_profile` tools — superseded by `get_config`/`set_config`. The MCP server now exposes 6 tools (previously 12) — the target surface reached.
- The `iam://workflow-examples` resource — its example workflows relied on the removed generation surface. The MCP server now exposes 6 resources (previously 7).
- The `sse` MCP transport (`--transport sse`) — removed, not deprecated: MCP revision 2026-07-28 defines only `stdio` and Streamable HTTP. `--transport` now only accepts `stdio`/`http` (default `stdio`); passing `sse` fails with a message naming `http` as the replacement.
- `iam-validator mcp`'s `--verbose`/`-v` flag — superseded by the global `--log-level` flag. The command's own `--config` pre-check and `ConfigLoader` summary print are also gone, since config validity is now enforced once at server startup for both entry points.
...
- `iam_validator.mcp.context.server_lifespan`: `await prewarm(context)` sat outside the lifespan's `try`/`finally`, so a `prewarm()` failure left the fetcher's underlying httpx client never closed. `prewarm()` and instructions setup now run inside the `try`, so the fetcher is always closed on the way out.
- Both MCP entry points printed a raw Python traceback on a bad `--config`. `resolve_settings()` now runs inside each entry point's own error handling, so both name the problem and exit non-zero instead.
