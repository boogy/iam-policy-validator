...
- `policy_type_validation` is now a registered `PolicyCheck` (was a standalone always-on function): it appears in the check inventory and supports the standard config system (enable/disable, severity, `ignore_patterns`). Side effect: the SCP 5,120-char size check now receives `raw_policy_dict` and actually fires (the old direct call never passed it).
