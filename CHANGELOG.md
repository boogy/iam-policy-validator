...
- Fix `_log_resolved_policy_type` debug log leaking absolute file paths — log the filename via `Path(policy_file).name` so `/home/<user>/…` no longer appears in `--log-level debug` output, and pre-format the message through an `isEnabledFor(DEBUG)` guard to clear CodeQL `py/clear-text-logging-sensitive-data` false positives on the per-policy type-resolution log line
