...
- `iam_validator.mcp.component_spec.ComponentSpec` (and its `ToolSpec`/`ResourceSpec`/`PromptSpec` subclasses): declarative gating metadata (`tag`, `modes`, `transports`, `scopes`, `mutating`) shared by tools, resources, and prompts, so a profile or transport restriction can't be bypassed by reaching an equivalent resource instead of a gated tool. `iam_validator.mcp.build.build_server(settings)` filters every spec through `spec_survives()` and registers survivors on a fresh `FastMCP` instance (never a module-level singleton), iterating each tool module's `TOOLS` list in a fixed order so repeated calls with identical settings produce an identical tool-name sequence. `read-only` filters on `mutating` instead of `tag`. `iam_validator.mcp.resources`/`iam_validator.mcp.prompts` (empty `RESOURCES`/`PROMPTS` lists) are placeholders; porting server.py's resources/prompts is TASK-05's job. No tool module defines `TOOLS` yet, so `build_server()` currently registers zero tools; it is not wired into the run path.
- `iam_validator.mcp.instructions`: `BASE_INSTRUCTIONS`/`get_instructions()` moved out of `server.py` so `build.py` can read them without importing `server.py` and its side effects (a second `FastMCP` instance, 24 legacy tool registrations). `server.py` re-exports both names unchanged.
...
- `iam_validator.mcp.server.apply_profile()` and its `mcp._transforms` snapshot poke — decorator-driven tag enable/disable, superseded by `build.build_server()`'s declarative `ComponentSpec` filtering. The CLI's `--profile` flag still only records the active profile (`set_active_profile()`) until it's wired to `build_server()`.
