...
from iam_validator.core.models import ServiceDetail, Statement, ValidationIssue
...
def clear_resource_support_cache() -> None:
    """Clear the action resource support cache.
...
class WildcardResourceCheck(PolicyCheck):
    ...
    async def _determine_severity_adjustment(
        ...
        """
        condition_keys = extract_condition_keys_from_statement(statement, restrictive_only=True)
    ...
    async def _filter_actions_requiring_resources(self, actions: list[str], fetcher: AWSServiceFetcher) -> list[str]:
        ...
        Access level (list/read/write/...) is not a proxy for this: some list-level
        actions take a resource ARN (e.g. s3:ListBucket) and must still be flagged.

        Examples of actions filtered out (no resource types defined for the action):
        - iam:ListUsers
        ...
        for action in actions:
            ...
            if action in _action_resource_support_cache:
